...
class ExperienceBuffer:
    ...
    def sample(self, batch_size):
        ...
        return np.array(states), np.array(actions), np.array(rewards, np.float32), np.array(dones, np.uint8), np.array(next_states)
...
def main():
    ...
    net.load_state_dict(torch.load(args.env + '-best.dat'))
